...
// *************************** 例程测试说明 ***************************
// 1.本例程会通过 Debug 串口输出测试信息 并且通过公共内存访问 使用CPU7_1发送图像数据 请务必接好调试串口以便获取测试信息
//
...
#pragma location = 0x2802bf80                                                   // 将下面这个数组定义到指定的RAM地址(只会强制指定下面的一个定义)
                                                                                // 这个地址可以直接在map文件中找到 map文件会在编译之后自动生成到Output文件夹内
...
int main(void)
{
    ...
    while(true)
    {
        ...
        if(mt9v03x_finish_flag)
        {
            ...
            if(image_fresh_flag == 0)
            {                
                memcpy(image_copy[0], mt9v03x_image[0], MT9V03X_IMAGE_SIZE);    // 在发送前将图像备份再进行发送，这样可以避免图像出现撕裂的问题
                SCB_CleanInvalidateDCache_by_Addr(image_copy[0], MT9V03X_IMAGE_SIZE);  // 写入备份数组之后，刷新同步RAM数据
