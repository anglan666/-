...
#define HALL_PIN                (P07_4)                                         // 定义霍尔触发引脚
      
#define LED1                    (P19_0)                                         // 定义板载LED引脚
...
int main(void)
{
    ...
    exti_init(HALL_PIN, EXTI_TRIGGER_RISING);                                   // 初始化霍尔引脚为上升沿触发              

    gpio_init(LED1, GPO, GPIO_HIGH, GPO_PUSH_PULL);                             // 初始化板载LED
    ...
    while(true)
    {
       ...
       if(1 == trigger_state)                                                   // 当霍尔引脚触发时 会在isr.c的外部中断服务函数将 trigger_state 置位    
       {
           trigger_state = 0;                                                   // 清除置位状态
           
           gpio_toggle_level(LED1);                                             // 翻转板载LED
...
void gpio_7_exti_isr()
{
    ...
    if(exti_flag_get(P07_4))				// 示例P07_4端口外部中断判断
    {
        trigger_state = 1;
