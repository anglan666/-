...
// 备注信息          检测方法: 累计100ms内运行该函数的耗时从而以100ms为周期计算CPU执行其他程序的耗时 计算出占用率
//                使用方法: 直接将该函数放主循环即可
//-------------------------------------------------------------------------------------------------------------------
void cpu_usage_check(void)
{
    ...
    system_count = timer_get(TC_TIME2_CH0);      // 获取定时器计数值

    if(system_count >= 100000)                   // 如果大于100ms 则进行占用率计算
    {
        timer_clear(TC_TIME2_CH0);               // 重置统计周期
    ...
    run_time_get = timer_get(TC_TIME2_CH0);                      // 统计运算该函数的耗时

    // 单次函数运算耗时累计 0.4us 为单次矫正偏差(CYT4BB7核心板测试数据)
    // 获取方法：主循环仅cpu_usage_check时，通过调节 偏差矫正值 保证占用率计算判断内的run_time_count在 100ms ± 1ms 即可
    run_time_count = run_time_count + ((float)run_time_get - (float)system_count + 0.4);
}
